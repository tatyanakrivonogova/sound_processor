...
int inputThread::input() {
	...
	if (thread.getHeader().get_audio_format() != 1) {
	...
	if (thread.getHeader().get_num_channels() != 1) {
	...
	if (thread.getHeader().get_sample_rate() != 44100) {
	...
	if (thread.getHeader().get_bits_per_sample() != 16) {
